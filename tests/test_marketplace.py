...
class TestList:
    ...
    def test_list_hides_platform_tools_by_default(
    ...
    def test_list_include_platform_shows_all(
        self, runner: CliRunner, patch_get_client
    ):
        """With --include-platform, platform tools appear."""
        result = runner.invoke(
            app, ["marketplace", "list", "--include-platform"]
        )
        assert result.exit_code == 0, result.output
    ...
    def test_list_json_with_include_platform(
        self, runner: CliRunner, patch_get_client
    ):
        import json

        result = runner.invoke(
            app, ["marketplace", "list", "--json", "--include-platform"]
        )
        assert result.exit_code == 0, result.output
        data = json.loads(result.output)
        names = [d["name"] for d in data]
...
class TestSearch:
    ...
    def test_search_does_not_return_platform_by_default(
        self, runner: CliRunner, patch_get_client
    ):
    ...
    def test_search_with_include_platform(
        self, runner: CliRunner, patch_get_client
    ):
        result = runner.invoke(
            app, ["marketplace", "search", "transmute", "--include-platform"]
        )
        assert result.exit_code == 0, result.output
        assert "platform/transmute" in result.output
...
class TestInfo:
    ...
    def test_info_platform_shows_builtin_message(
        self, runner: CliRunner, patch_get_client
    ):
        """Info for a platform tool shows the built-in notice."""
        result = runner.invoke(
            app, ["marketplace", "info", "platform/transmute"]
        )
        assert result.exit_code == 0, result.output
        assert "platform/transmute" in result.output
...
class TestSubscribe:
    ...
    def test_subscribe_platform_tool_fails_early(
        ...
        result = runner.invoke(
            app, ["marketplace", "subscribe", "platform/transmute", "--yes"]
        )
        assert result.exit_code == 1

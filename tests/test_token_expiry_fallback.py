...
async def test_handle_token_expiry_retry_on_email_present():
    ...
    with patch("teardrop_cli.config.load_config") as mock_load, \
         patch("teardrop_cli.config.has_existing_credentials") as mock_has, \
         patch("teardrop_cli.config._keyring_available") as mock_key_avail, \
         patch("keyring.get_password") as mock_pass:
